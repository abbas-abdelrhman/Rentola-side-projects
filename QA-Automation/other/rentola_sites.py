...
class QaAutomationSpider(scrapy.Spider):
    name = "qa_automation"

    start_urls = [
        # Monday - Week 1 & Week 3 | rentola.com && findallrentals.com
        # {
        #     "url": "https://findallrentals.com/for-rent?rent_per=month&order=rent_desc",
        #     "allowed_domains": "findallrentals.com"},
        # {
        #     "url": 'https://rentola.com/for-rent?rent_per=month&order=rent_desc',
        #     "allowed_domains": "rentola.com"
        # },

        # Friday - Week 2 & Week 4 | rentola.nl && huurportaal.nl && rentwyre.com
        {
    ...
    def parse(self, response, **kwargs):
        ...
        # pagination = response.xpath("//div[@class='hidden pagination']/a[@class='next-page']/@class").get()

        if next_page:
            yield scrapy.Request(url="https://rentwyre.com/for-rent?order=area_desc&amp;page=2",
                                 callback=self.parse,
                                 headers=self.headers,
                                 cookies=sites_cookies[allowed_domains],
    ...
    def populate_item(self, response):
        ...
        source = response.xpath('//ul[@class="list list-inline"]/li[contains(.,"Source:")]/a/@href').get()
        ...
        yield {
            "domain": domain,
            "rentola": response.url,
            "source": source,
            "area": area,
            "images_length": images_length,
            "bedrooms": bedrooms,
            "bathrooms": bathrooms,
            "prop_price": int(extract_number_only(response.meta['prop_price'])),
