...
import scrapy
from item2 import DownfilesItem
from scrapy.crawler import CrawlerProcess

# def usa_domains():
#     dom = []
#     with open('/home/abbas/PycharmProjects/QA-Automation/data/USA/all_USA_ads.json', "r") as file:
#         for domain in json.loads(file.read()):
#             if domain.get('domain', [''])[0]:
#                 dom.append(domain.get('domain', [''])[0])
#
#     return set(dom)
#
# print(usa_domains())
...
class SpiderNest(scrapy.Spider):
    ...
    custom_settings = {
        ...
        "FILES_STORE": "/home/abbas/PycharmProjects/QA-Automation/data/USA/US-DOMAINS",
    }
    ...
    def submission_page(self, response):
        ...
        records = country = name = sub_status = json_file_data = None
        for item in response.xpath('//table[@class="table"]//tr'):
            ...
            json_file_data = item.xpath('.//a[@title="Download JSON"]/@href').get()

        if json_file_data:
            json_file_data = "https://c.findboliger.dk{}".format(json_file_data)
            item = DownfilesItem()
            item['file_urls'] = [json_file_data]
            yield item
...
if __name__ == "__main__":
    process = CrawlerProcess(settings={
        "FEEDS": {
            "/home/abbas/PycharmProjects/QA-Automation/data/USA/US-DOMAINS/test.json": {
                "format": "json",
                "overwrite": True,
                'encoding': 'utf8',
            },
